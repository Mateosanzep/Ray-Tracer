...
public class Raytracer {
    ...
    public Color castRay(Ray ray) {
        ...
        for (Light light : scene.getLights()) {
            Vector3D L = light.position.VectorSubstract(hitPoint).Normalize();

            Vector3D lightDir = new Vector3D(-light.direction.X, -light.direction.Y, -light.direction.Z).Normalize();
